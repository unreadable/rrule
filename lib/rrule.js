...
;(function (root, factory) {
  ...
  RRule.prototype = {
    ...
    */
    ...
    _iter: function (iterResult, rrule) {
        ...
        if (plb(bysetpos) && plb(timeset)) {
          ...
          for (j = 0; j < poslist.length; j++) {
            ...
            if (until && res > until) {
              ...
              ++total
              if (!iterResult.accept(res, rrule)) return iterResult.getValue()
              if (count) {
                --count
                if (!count) {
                  this._len = total
  ...
  var CallbackIterResult = function (method, args, iterator) {
    ...
    this.add = function (date, rrule) {
      if (iterator(date, this._result.length, rrule)) {
        this._result.push(date)
        return true
      }
      return false
    ...
    */
    ...
    _iter: function (iterResult) {
      ...
      if (iterResult.method === 'between') {
        ...
        iterResult.accept = function (date, rrule) {
          var dt = Number(date)
          if (!_exdateHash[dt]) {
            _exdateHash[dt] = true
            return _accept.call(this, date, rrule)
          }
          return true
        }
      }
